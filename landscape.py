...
def main(problem_number):
    ...
    if 'routing' in problem_dict:
        del problem_dict['noise']
        _run_landscape_routing(**problem_dict)
    elif problem_dict['noise']:
        del problem_dict['noise']
...
def _run_landscape_routing(W,W_max,p,routing):
    ...
    print('Creating landscape with routing',routing,'for', str(S))
    start_time = time.monotonic()
    gammas, betas, exp_costs = landscape_routing(J, h, routing, costs, step_size=0.01, shots=5000)
